  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
       <div className="w-full h-80 relative overflow-hidden bg-slate-900">
         {charity.image_url && (
            <img src={charity.image_url} alt={charity.name} className="absolute inset-0 w-full h-full object-cover opacity-60" />
         )}
         <div className="absolute inset-0 bg-gradient-to-t from-slate-900/90 to-transparent"></div>
         <div className="absolute bottom-10 left-10 md:left-24 max-w-4xl z-10">
           {charity.category && <span className="bg-emerald-500 text-white px-3 py-1 rounded-full text-sm font-bold shadow-sm mb-4 inline-block">{charity.category}</span>}
           {charity.featured && <span className="bg-amber-500 text-white px-3 py-1 rounded-full text-sm font-bold shadow-sm mb-4 ml-2 inline-block">Featured</span>}
           <h1 className="text-4xl md:text-6xl font-extrabold text-white tracking-tight">{charity.name}</h1>
           {charity.website && (
              <a href={charity.website} target="_blank" rel="noopener noreferrer" className="text-emerald-400 hover:text-emerald-300 font-semibold mt-4 block">
                {charity.website} ↗
              </a>
           )}
         </div>
       ...
       <div className="max-w-4xl mx-auto px-8 py-16 grid grid-cols-1 md:grid-cols-3 gap-12">
          <div className="md:col-span-2">
            ...
            <div className="mt-16 bg-emerald-50 rounded-2xl p-8 border border-emerald-100 shadow-sm">
               <h3 className="text-xl font-bold text-emerald-900 mb-2">Want to support {charity.name}?</h3>
               ...
               {charity.website && (
                  <a href={charity.website} target="_blank" rel="noopener noreferrer" className="ml-4 text-emerald-600 font-semibold hover:text-emerald-500 underline">Donate externally Instead</a>
               )}
            </div>
          </div>
          ...
          <div className="md:col-span-1 border-l pl-0 md:pl-12 border-slate-200">
             ...
             {charity.events && charity.events.length > 0 ? (
                <div className="space-y-6">
                  {charity.events.map((ev: any, idx: number) => (
