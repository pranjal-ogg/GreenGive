...
export default async function AdminDraw() {
  ...
  const { data: latestDraw } = await supabaseAdmin.from('draws').select('*').eq('status', 'simulated').order('created_at', { ascending: false }).limit(1).maybeSingle()

  const typedDraws = draws as Draw[] | null
  const typedLatestDraw = latestDraw as Draw | null
  ...
  async function runSimulation(formData: FormData) {
    ...
    const fee = activePlans?.reduce((sum, s) => sum + (s.plan === 'yearly' ? 100 / 12 : 10), 0) || 0
    const subsCount = activePlans?.length || 0

    const prizes = calculatePrizes(fee, 0, { match5: 0, match4: 0, match3: 0 })
  ...
  async function publishDraw(formData: FormData) {
    ...
    // Match against users' current scores
    ...
    if (typedDraw) {
      for (const [userId, userNums] of Object.entries(byUser)) {
        const matchCount = calculateMatches(typedDraw.winning_numbers, userNums)
        if (matchCount >= 3) {
          ...
          <div className="bg-slate-900 border border-indigo-500/30 rounded-2xl p-6">
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                <p className="text-slate-500 text-sm">Type: {typedLatestDraw.draw_type} · {new Date(typedLatestDraw.created_at).toLocaleString()}</p>
              </div>
            ...
            <div className="grid grid-cols-3 gap-3 mb-6">
              <div className="bg-slate-800/60 rounded-xl p-3 text-center">
                ...
                <div className="text-emerald-400 font-bold text-sm">£{Number(typedLatestDraw.jackpot_amount).toFixed(2)}</div>
              </div>
              <div className="bg-slate-800/60 rounded-xl p-3 text-center">
                <div className="text-xs text-slate-500 mb-1">4-match Pool</div>
                <div className="text-indigo-300 font-bold text-sm">£{Number(typedLatestDraw.pool_4match).toFixed(2)}</div>
              </div>
              <div className="bg-slate-800/60 rounded-xl p-3 text-center">
                <div className="text-xs text-slate-500 mb-1">3-match Pool</div>
                <div className="text-slate-300 font-bold text-sm">£{Number(typedLatestDraw.pool_3match).toFixed(2)}</div>
              </div>
            </div>

            <form action={publishDraw}>
              <input type="hidden" name="drawId" value={typedLatestDraw.id} />
